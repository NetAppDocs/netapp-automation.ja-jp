...
summary: この記事は、NetApp Console Automation ハブからNetApp のお客様が利用できるNetApp Cloud Volumes ONTAP for Google Cloud Automation ソリューションをサポートしています。 
---
...
[role="lead"]
この記事は、NetApp Console自動化ハブからNetApp のお客様が利用できる、Google Cloud Automation ソリューション向けのNetApp Cloud Volumes ONTAP をサポートしています。
...
.開始する前に
* ダウンロードする必要があります link:https://console.netapp.com/automationHub["Cloud Volumes ONTAP for Google Cloud -クラウドへのバースト"^] コンソール Web UI を介した自動化ソリューション。このソリューションは次のようにパッケージ化されています。 `cvo_gcp_flexcache.zip`。
