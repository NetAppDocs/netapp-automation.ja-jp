...
* ダウンロードする必要があります https://console.netapp.com/automationHub["Amazon FSx for NetApp ONTAP管理 - クラウドへのバースト"^] NetApp ConsoleWeb UI を介した自動化ソリューション。ソリューションはファイルとしてパッケージ化されています `AWS_FSxN_BTC.zip`。
